...
// 2014.11.07 Ver1.00[CSuperGridCtrl Ctrl+C Copy Implement] Start
#define IDR_ACCELERATOR1                130
#define VK_C							0x43
// 2014.11.07 Ver1.00[CSuperGridCtrl Ctrl+C Copy Implement] End
...
// 2014.11.07 Ver1.00[CSuperGridCtrl Ctrl+C Copy Implement] Start
	if(pMsg->message >= WM_KEYFIRST && pMsg->message <= WM_KEYLAST)
	{
		...
		if(haccel && ::TranslateAccelerator(m_hWnd, haccel, pMsg))
		{
			if(pMsg->wParam == VK_C)
			{
				int nRow = GetSelectedItem();
				int nCol = GetCurSubItem();
				...
				memcpy(pBuffer, strValue.GetBuffer(0), nLen);
				*((TCHAR*)pBuffer + nLen) = 0;
