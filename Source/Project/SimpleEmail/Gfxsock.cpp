...
// 2015.03.03 Ver10.9.0.2 [Menu/Send Mail Failed] End
{
	...
	memset(pszBuffer, 0x00, nLen);
	if(nFlag == RECVUNBLOCK)
	{
// 2015.03.03 Ver10.9.0.2 [Menu/Send Mail Failed] Start
		if(nRepeat >= REPEATMAX)
		{
			return -1;
		}
// 2015.03.03 Ver10.9.0.2 [Menu/Send Mail Failed] ENd
		UnBlockSocket(s);
		nRet = recv(s, pszBuffer, nLen, 0);
// 2015.03.03 Ver10.9.0.2 [Menu/Send Mail Failed] Start
		if (nRet == -1)
		{
			...
			{
				BlockSocket(s);
				return receive(s, pszBuffer, nLen, nFlag, nRepeat++);
			}
		}
// 2015.03.03 Ver10.9.0.2 [Menu/Send Mail Failed] End
		BlockSocket(s);
	}
	else if(nFlag == RECVBLOCK)
	{
		SetRecvTimeo(s, RECV_TIMEO);
		nRet = recv(s, pszBuffer, nLen, 0);
	}
